...
| `-c`, `--config` | Path to a fastp config file (one parameter per line). | `default_config` | If `default_config`, reads `data/Configs/fastp_default.config`.<br> Default values are: --cut_tail, --cut_window_size=10, --cut_mean_quality=20, --length_required=50, --correction|
...
- `--kna`keeps the VarScan and Mutect2 outputs before being filtered by annotation

<a id="annotation_filter"></a>
ANNOTATION FILTER (PipeModules/AnnotationFilter.py)
-------------------------------------------------

**WATCH OUT** this module is now included in the Calling module
...
**Inputs needed, produced by the Calling pipeline:**
- `{prefix}.EPI.snp.final.annoF` 
- `{prefix}.final_sin_wt_complemented_annotSnpEff.vcf` 
- `{prefix}.wt` 
- `{prefix}.indel.vcf`
- `{prefix}.snp.minos`
- `{prefix}.lowcov`
...
- `ztemp_individual_fastas` — This folder contain the individual fasta files generated if the script stops due to an error. This way we don't need to generate all the files again. 
...
|---|---|---:|---|
| `-p`, `--prefix` | If provided, analyze the DR file and produce `{prefix}.res` | 
